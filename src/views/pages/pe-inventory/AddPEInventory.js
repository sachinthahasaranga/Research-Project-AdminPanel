...
const AddPEInventory = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.post('/api/inventory/pe/add', formData, { withCredentials: true })

      // Show success alert
      Swal.fire({
        icon: 'success',
        title: 'PE Inventory Added!',
        text: 'The new PE inventory item has been successfully added.',
      ...
      console.error('Error adding PE inventory:', error)

      // Show error alert
      Swal.fire({
        icon: 'error',
        title: 'Failed!',
        text: 'An error occurred while adding the PE inventory item.',
    ...
    <CRow>
      <CCol xs={12} md={12} lg={12} className="mx-auto">
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Add New PE Inventory</strong>
          </CCardHeader>
          <CCardBody>
