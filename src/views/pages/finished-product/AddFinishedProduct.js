...
const AddFinishedProduct = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.post('/api/inventory/finished-product/add', formData, { withCredentials: true })

      // Show success alert
      Swal.fire({
        icon: 'success',
        title: 'Finished Product Added!',
        text: 'The new finished product has been successfully added.',
      ...
      Swal.fire({
        icon: 'error',
        title: 'Failed!',
        text: 'An error occurred while adding the finished product.',
    ...
    <CRow>
      <CCol xs={12} md={12} lg={12} className="mx-auto">
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Add New Finished Product</strong>
          </CCardHeader>
          <CCardBody>
